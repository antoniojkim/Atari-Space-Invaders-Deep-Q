        ...
        if len(self.memory) > 0:
            ...
            if len(batches) > 0:
                ...
                for batch_input, batch_label in batches:
                    input_tensor = torch.from_numpy(batch_input.astype(np.double)).double().to(self.device)
                    label_tensor = torch.from_numpy(batch_label.astype(np.double)).double().to(self.device)
